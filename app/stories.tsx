    ...
    <section className="w-full">
      <div className="flex gap-4 overflow-x-auto pb-4">
        {items.map((item: StoriesType, index) => (
          <button
            key={index}
            className="flex flex-shrink-0 flex-row items-center justify-center gap-2 whitespace-nowrap rounded-full bg-gradient-to-r from-rose-500 to-rose-700 py-1 px-4 text-base font-semibold text-white outline outline-[5px] outline-rose-500/30 transition-all duration-300 hover:from-rose-700 hover:to-rose-500"
            onClick={() => {
          ...
          >
            ...
            <svg
              onClick={(e) => {
                e.stopPropagation() // Prevent the button's main onClick from firing
                deleteStorage(item)
